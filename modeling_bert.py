...
def getgraphvec():
    f = open('/home/hjw19/miniconda3/envs/pytorch/lib/python3.7/site-packages/transformers/node2vec_256.pkl','rb')  # 256
    t = open('/home/hjw19/miniconda3/envs/pytorch/lib/python3.7/site-packages/transformers/node2vec_512.pkl','rb')  # 512
    data1 = pickle.load(f)
    d = torch.Tensor(data1['solver']['vertex_embeddings'])
    data2 = pickle.load(t)
    c = torch.Tensor(data2['solver']['vertex_embeddings'])

    g = torch.cat((d,c),dim=1)  #[1682,768]
    return g
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        # print(inputs_embeds.size())  # [6,512,768]
        # print(position_embeddings.size()) # [6,512,768]
        embeds = nn.Embedding(1682, 768)
        g = getgraphvec()
        pretrained_weight = np.array(g) # 已有词向量的numpy形式
        embeds.weight.data.copy_(torch.from_numpy(pretrained_weight)) # from_numpy将其转化为tensor
        h = embeds(Variable(torch.LongTensor(input_ids)))
        print('*****')
        print(h.size())
        print('*****')
        embeddings = inputs_embeds + position_embeddings + token_type_embeddings + h
